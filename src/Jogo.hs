...
picturesPattern _ [] = []
picturesPattern imagens (x:xs) = case lookup x imagensContexto of
  Just img -> scale 0.08 0.08 img : picturesPattern imagens xs
  Nothing  -> Blank : picturesPattern imagens xs
  where
    imagensContexto = map (\(c, p) -> (Just c, p)) imagens

picturesParede :: [(Cor, Picture)] -> LinhaParede -> [Picture]
picturesParede _ [] = []
picturesParede imagens ((c, True):xs) = case lookup c imagens of
  Just img -> scale 0.08 0.08 img : picturesParede imagens xs
  Nothing  -> Blank : picturesParede imagens xs
picturesParede imagens ((_, False):xs) = Blank : picturesParede imagens xs
